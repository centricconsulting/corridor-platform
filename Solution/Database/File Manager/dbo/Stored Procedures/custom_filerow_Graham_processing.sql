...
CREATE PROCEDURE [dbo].[custom_filerow_Graham_processing] (@agency_file_key as int, @file_name as varchar(200))
AS
...
-- Product Suffix warnings
UPDATE agency_file_row
SET process_dtm = GETDATE(), process_success_ind = 0, process_error_category = 'WARNING', process_error_message = 'FILE:' + @file_name + ', MRN:' + column06 + ', Record Rejected - Product:' + column17, create_agency_medical_record_ind = 0, notification_sent_ind = 0
WHERE agency_file_key = @agency_file_key
AND process_dtm IS NULL
AND row_index > @header_row_index
...
(
	...
	UNION ALL
	SELECT 'ROC/RECERT', 'RECERT' -- ROC/RECERT are always handled as RECERT
	UNION ALL
...
-- Only one live location right now
UPDATE agency_file_row
SET process_dtm = GETDATE(), process_success_ind = 0, process_error_category = 'WARNING', process_error_message = 'FILE:' + @file_name + ', MRN:' + column06 + ', Record Rejected - NON-NPR Location', create_agency_medical_record_ind = 0, notification_sent_ind = 0
WHERE agency_file_key = @agency_file_key
AND process_dtm IS NULL
AND row_index > @header_row_index
AND column02 != 'NPR'
...
UPDATE agency_file_row
SET process_dtm = GETDATE(), process_success_ind = 0, process_error_category = 'WARNING', process_error_message = 'FILE:' + @file_name + ', MRN:' + column06 + ', Record Rejected - Responsible Position: ' + column09, create_agency_medical_record_ind = 0, notification_sent_ind = 0
WHERE agency_file_key = @agency_file_key
AND process_dtm IS NULL
AND row_index > @header_row_index
AND column09 != 'Quality Assurance'
AND column09 != 'Quality Assurance 2'
AND column09 != 'Clinical Field Staff Supervisor'
