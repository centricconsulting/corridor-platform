...
CREATE PROCEDURE [dbo].[custom_sfclookup_Pathways] (@agency_key as int, @process_batch_key as int)
AS
...
INNER JOIN
(
	SELECT agency_medical_record_key, 'Extended OASIS Review SOC' Product FROM agency_medical_record
	WHERE oasis_visit_type LIKE '%START OF CARE%'
	UNION ALL
	SELECT agency_medical_record_key, 'Extended OASIS Review Recert' Product FROM agency_medical_record
	WHERE oasis_visit_type LIKE '%RECERTIFICATION%'
) Product_Names
ON agency_medical_record.agency_medical_record_key = Product_Names.agency_medical_record_key
INNER JOIN sfc.DM_Agency_Product_Rate__c
ON Product_Names.Product = sfc.DM_Agency_Product_Rate__c.Product_Name_for_IT__c
...
UPDATE agency_medical_record
SET process_dtm = GETDATE(), process_success_ind = 0, process_error_message = 'FILE:' + [file_name] + ', MRN:' + medical_record_number + ', Cannot find Product Rate for "' + oasis_visit_type + '" in HHCP', salesforce_send_ind = 0, notification_sent_ind = 0
FROM agency_medical_record
INNER JOIN agency_file_row
ON agency_medical_record.agency_file_row_key = agency_file_row.agency_file_row_key
INNER JOIN agency_file
