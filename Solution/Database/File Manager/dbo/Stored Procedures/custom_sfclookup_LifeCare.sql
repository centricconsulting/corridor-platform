CREATE PROCEDURE [dbo].[custom_sfclookup_LifeCare] (@agency_key as int, @process_batch_key as int)
AS
...
AND sfc.Form_to_product_lookup__c.Parent_Agency__c = @TrueParentAgencyID
...
-- Form lookup error messages
UPDATE agency_medical_record
SET process_dtm = GETDATE(), process_success_ind = 0, process_error_message = 'FILE:' + [file_name] + ', MRN:' + medical_record_number + ', Cannot find Form to Lookup for "' + oasis_visit_type + '" in HHCP', salesforce_send_ind = 0, notification_sent_ind = 0
FROM agency_medical_record
INNER JOIN agency_file_row
ON agency_medical_record.agency_file_row_key = agency_file_row.agency_file_row_key
INNER JOIN agency_file
...
AND oasis_visit_type NOT IN
(
	SELECT Form__c FROM sfc.Form_to_product_lookup__c
	WHERE Parent_Agency__c = @TrueParentAgencyID
	AND Form__c IS NOT NULL
)

-- Product rate error messages
UPDATE agency_medical_record
SET process_dtm = GETDATE(), process_success_ind = 0, process_error_message = 'FILE:' + [file_name] + ', MRN:' + medical_record_number + ', Cannot find Product Rate for "' + agency_location + '+' + Product_Code__c + '" in HHCP', salesforce_send_ind = 0, notification_sent_ind = 0
FROM agency_medical_record
INNER JOIN agency_file_row
ON agency_medical_record.agency_file_row_key = agency_file_row.agency_file_row_key
INNER JOIN agency_file
