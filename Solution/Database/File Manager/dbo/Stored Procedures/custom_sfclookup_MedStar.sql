...
ON agency_medical_record.agency_medical_record_key = AliasCalculation.agency_medical_record_key
inner join [sfc].[DM_Agency__c] 
ON AliasCalculation.AgencyAlias = [sfc].[DM_Agency__c].Agency_Alias__c
WHERE agency_medical_record.process_batch_key = @process_batch_key
AND agency_key = @agency_key
AND salesforce_send_ind = 1
