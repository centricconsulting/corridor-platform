...
BEGIN
  ...
  -- update semi-month index
  SELECT 
    semi_month_key
  , row_number() OVER (ORDER BY semi_month_key) AS semi_month_basis
  , CASE
    WHEN @current_dt BETWEEN semi_month_start_dt AND semi_month_end_dt THEN 1 
    ELSE 0 END AS current_ind
  INTO #tmp_semi_month
  FROM
  calendar c
  WHERE day_of_month IN ( 1, 16)
  OR date_key IN (0,99999999)
  ;

  UPDATE c
  SET c.semi_month_index = (b.semi_month_basis - bc.semi_month_basis)
  FROM
  calendar c
  INNER JOIN #tmp_semi_month b ON b.semi_month_key = c.semi_month_key
  LEFT JOIN #tmp_semi_month bc ON bc.current_ind = 1
  WHERE
  c.date_key NOT IN (0,99999999)
  ;
